...
- **Key rotation:** When rotating keys, follow a safe procedure: decrypt with the old key, then re-encrypt with the new key (e.g. using the bundle’s decrypt/encrypt commands in a controlled way). See [Key rotation](KEY_ROTATION.md).
